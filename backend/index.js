...
// Configure multer for complaint images using memory storage
...
app.put('/api/sellers/:id', upload.single('image'), async (req, res) => {
    try {
        let updateData = { ...req.body };
        
        // If a new image was uploaded, add its path to updateData
        if (req.file) {
            updateData.image = `/uploads/sellers/${req.file.filename}`;
        }
...
app.post('/api/products', productUpload.single('image'), async (req, res) => {
    try {
        ...
        const productData = {
            ...req.body,
            price_per_unit: Number(req.body.price_per_unit),
            stock: Number(req.body.stock),
            subcategory: req.body.subcategory || req.body.category // Fallback to category if subcategory not provided
        };
        ...
        // Return the complete product data with full image URL
        const savedProduct = await Product.findById(product._id).populate('seller_id');
...
app.put('/api/products/:id', productUpload.single('image'), async (req, res) => {
  try {
    let updateData = { ...req.body };
    
    // Convert numeric strings to numbers
    if (updateData.price_per_unit) {
      updateData.price_per_unit = Number(updateData.price_per_unit);
    }
    if (updateData.stock) {
    ...
    // Send the complete updated product data
...
//get the buyer list of a specific seller   
app.get('/api/sellers/:sellerId/buyers', async (req, res) => {
    try {
        // Find all orders for this seller and populate buyer details
        const orders = await Order.find({ seller_id: req.params.sellerId })
        ...
        orders.forEach(order => {
            if (order.buyer_id && !buyerMap.has(order.buyer_id._id.toString())) {
                buyerMap.set(order.buyer_id._id.toString(), order.buyer_id);
            }
        });
 ...
 */
// Post buyer complaint
app.post('/api/buyer-complaints', complaintUpload.single('image'), async (req, res) => {
    try {
        let imageUrl = null;
        ...
        const complaint = new BuyerComplaint({
            complaint_id: req.body.complaint_id,    // seller being complained about
            accuser_id: req.body.accuser_id,       // buyer making the complaint
        ...
        // Populate the saved complaint with seller details
        const populatedComplaint = await BuyerComplaint.findById(complaint._id)
            .populate('complaint_id')
            .populate('accuser_id');
            
        res.status(201).json(populatedComplaint);
    } catch (error) {
        console.error('Error creating buyer complaint:', error);
        res.status(400).json({ error: error.message });
    }
});
...
//seller complaint
app.post('/api/complaints/seller', complaintUpload.single('image'), async (req, res) => {
    try {
        let imageUrl = null;
        ...
        const complaint = new SellerComplaint({
            complainant_id: req.body.complainant_id, // buyer being complained about
            accuser_id: req.body.accuser_id,         // seller making the complaint
            message: req.body.message,
            image: imageUrl,
            status: 'PENDING'
        });
        ...
        // Populate the saved complaint with complainant details
        const populatedComplaint = await SellerComplaint.findById(complaint._id)
            .populate('complainant_id')
            .populate('accuser_id');
            
        res.status(201).json(populatedComplaint);
    } catch (error) {
        console.error('Error creating seller complaint:', error);
        res.status(400).json({ error: error.message });
    }
});
...
// Get complaints for a specific seller
app.get('/api/complaints/seller/:sellerId', async (req, res) => {
    try {
        const complaints = await SellerComplaint.find({ accuser_id: req.params.sellerId })
            .populate('complainant_id')
            .populate('accuser_id')
            .sort({ createdAt: -1 });
        res.json(complaints);
